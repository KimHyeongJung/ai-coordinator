...
style: 클래식/스포티/포멀/캐주얼/미니멀 중 해당하는 것 배열 (복수 가능)
  스타일 판단 기준 (반드시 준수):
  캐주얼 → 청바지·데님(jeans·denim), 티셔츠(t-shirt), 후드·맨투맨(hoodie·sweatshirt), 스니커즈(sneakers), 반바지(shorts)
  스포티 → 트레이닝복·조거팬츠(jogger·training), 레깅스(leggings), 운동화·러닝화(running shoes)
  포멀  → 정장 바지·슈트(suit·dress pants), 블라우스(blouse), 드레스 셔츠(dress shirt), 구두(dress shoes)
  클래식 → 트렌치코트(trench coat), 블레이저(blazer), 폴로셔츠(polo), 슬랙스(slacks), 로퍼(loafer)
  미니멀 → 단색 기본 아이템, 심플한 디자인, 장식 없는 깔끔한 의류
  주의: 청바지·데님은 절대 클래식으로 분류하지 말 것 → 반드시 캐주얼
